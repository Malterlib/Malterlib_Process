...
namespace NMib
{
	...
	#define DProcessProxyProtocolType(d_Type, d_TypeID) \
			...
			{
				uint64 m_LaunchID;
				NContainer::TCStreamableVariant<EProcessLaunchState, uint8, EProcessLaunchState_Launched, NMib::NStr::CStr, EProcessLaunchState_LaunchFailed, uint32, EProcessLaunchState_Exited> m_State;
...
#define DProcessProxyProtocolNumTypeID 7
...
#			undef DMibTemp_GenerateParam
		...
		{
			NPtr::TCUniquePointer<NMib::NProcess::CProcessLaunch> m_pServer;
			NThread::CEvent m_ServerExited;
			
			NPtr::TCUniquePointer<NThread::CThreadObject> m_pThreadObject;
			...
			void f_SendPacket(NContainer::TCVector<uint8> const &_Data, NFunction::TCFunction<void (NStr::CStr const &_Error)> &&_fOnError)
			{
				NStr::CStr Base64Data = NDataProcessing::fg_Base64Encode(_Data);
				Base64Data += "\n";
			...
			void fp_Send(NContainer::TCVector<uint8> &&_Packet, NFunction::TCFunction<void (NStr::CStr const &_Error)> &&_fOnError)
			{
			...
			struct CLaunchState
			{
				...
				void f_ReportStateChange(NPtr::TCSharedPointer<CLaunchState> const &_pState, CProcessLaunchStateChangeVariant const &_State, fp64 _Time) const;
				void f_ReportLaunchFailed(NPtr::TCSharedPointer<CLaunchState> const &_pState, NStr::CStr const &_ErrorStr) const;
				void f_ReportOutput(NPtr::TCSharedPointer<CLaunchState> const &_pState, EProcessLaunchOutputType _OutputType, NMib::NStr::CStr const &_Output) const;
			};

			NThread::CMutual m_LaunchStatesLock;
			struct CLaunch
			{
				NPtr::TCSharedPointer<CLaunchState> m_pState;
			};
			NContainer::TCMap<uint64, CLaunch> m_LaunchStates;
			
			void f_AddLaunchState(uint64 _LaunchID, NPtr::TCSharedPointer<CLaunchState> const &_State)
			{
				DMibLock(m_LaunchStatesLock);
			...
			NPtr::TCSharedPointer<CLaunchState> f_GetLaunchState(uint64 _LaunchID)
			{
				DMibLock(m_LaunchStatesLock);
		...
		{
			return
				[this](CProcessLaunchParams const &_Params, EProcessLaunchCloseFlag _DestructFlags) -> NPtr::TCUniquePointer<CVirtualProcessLaunch>
				{
		...
		struct CProxiedLaunchServer::CInternal
		{
			...
			void f_SendPacket(NContainer::TCVector<uint8> const &_Data)
			{
				NStr::CStr Base64Data = NDataProcessing::fg_Base64Encode(_Data);
				Base64Data += "\n";
			...
			struct CLaunchState : public NPtr::TCSharedPointerIntrusiveBase<>
			{
				...
				NPtr::TCUniquePointer<CProcessLaunch> m_pLaunch;
			...
			NThread::CMutual m_LaunchStatesLock;
			NContainer::TCMap<uint64, NPtr::TCSharedPointer<CLaunchState>> m_LaunchStates;
		...
		namespace NPrivate
		{
			...
			{
				NPtr::TCSharedPointer<CProxiedLaunchServer::CInternal::CLaunchState> pState 
					= fg_Construct
					(
						_pServer
						, m_LaunchID
				...
				NPtr::TCSharedPointer<COnScopeExitShared> pCleanupLaunch = fg_Construct();
			...
			struct CHandleMessageVisitor
			{
				...
				NFunction::TCFunction<void (NContainer::TCVector<uint8> &&_SendReply)> m_fOnReply;
				...
				CHandleMessageVisitor
					(
						...
						, uint32 _MessageID
						, NFunction::TCFunction<void (NContainer::TCVector<uint8> &&_SendReply)> &&_fOnReply
					)
					: m_pClient(_pClient)
					, m_pServer(nullptr)
					, m_Stream(_Stream)
		...
		void CProxiedLaunchClient::CInternal::f_ReceivePacket(NStr::CStr const &_Packet)
		{
			...
			try
			{
				NDataProcessing::fg_Base64Decode(_Packet, Output);
			}
				...
				else
				{
					...
					NPrivate::CHandleMessageVisitor Visitor
						(
							...
							, Stream
							, MessageID
							, [this](NContainer::TCVector<uint8> &&_SendReply)
							{
		...
		{
			NPtr::TCSharedPointer<NAtomic::TCAtomic<smint>> pErrorHandled = fg_Construct();
			...
			NContainer::TCVector<uint8> ToSend = Stream.f_MoveVectorOptimized();
			fp_Send
				(
					fg_Move(ToSend)
		...
		void CProxiedLaunchClient::CInternal::CLaunchState::f_ReportLaunchFailed(NPtr::TCSharedPointer<CLaunchState> const &_pState, NStr::CStr const &_ErrorStr) const
		{
		...
		void CProxiedLaunchClient::CInternal::CLaunchState::f_ReportStateChange
			(
				NPtr::TCSharedPointer<CLaunchState> const &_pState
		...
		void CProxiedLaunchServer::CInternal::CLaunchState::f_SendStdOutput(EProcessLaunchOutputType _OutputType, NMib::NStr::CStr const &_Output)
		{
			if (!_Output.f_IsEmpty())
			{
				...
				if (Server.f_GetServer())
				{
					NPtr::TCSharedPointer<CLaunchState> pState2 = fg_Explicit(&fg_RemoveQualifiers(*this));
					Server.f_GetServer()->f_SendMessageAsync
						(
							Message
